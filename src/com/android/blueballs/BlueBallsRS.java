...
public class BlueBallsRS implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public void init(Context context, int dpi, RenderScriptGL rs,
        ...
        if (!mInited) {
            mDensityDPI = dpi;
            mContext = context;
            mSharedPref = mContext.getSharedPreferences(BlueBallsSelector.KEY_PREFS,
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        if (key.equals(BlueBallsSelector.KEY_ENABLED)) {
            loadTextures();
        }
        makeNewState();
    }
    ...
    private void makeNewState() {
        Float3 adjust;
        if (mSharedPref.getBoolean(BlueBallsSelector.KEY_ENABLED,
                    mRes.getBoolean(R.bool.recolor_enabled))) {
            adjust = new Float3(
                    mSharedPref.getFloat(BlueBallsSelector.KEY_HUE,
                        Float.valueOf(mRes.getString(R.string.hue))),
                    mSharedPref.getFloat(BlueBallsSelector.KEY_SATURATION,
                        Float.valueOf(mRes.getString(R.string.saturation))),
                    mSharedPref.getFloat(BlueBallsSelector.KEY_BRIGHTNESS,
    ...
    private void loadTextures() {
        boolean recolor = mSharedPref.getBoolean(BlueBallsSelector.KEY_ENABLED, false);
